...
const GlowCard = ({ children, className = "" }: { children: React.ReactNode, className?: string }) => {
  ...
  return (
    <div
      ...
      className={`relative overflow-hidden group rounded-2xl border border-slate-200 bg-white transition-shadow hover:shadow-lg hover:-translate-y-1 transform duration-300 ${className}`}
    >
      <div
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-100">
        ...
        <div className="container flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/login?tab=signup">
              <Button size="sm" className="rounded-md px-5 font-medium bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm transition-all hover:scale-105 active:scale-95">
                Get Started
              </Button>
            </Link>
          </div>
      ...
      <section className="pt-36 pb-20 px-4 relative z-10 overflow-hidden">
        ...
        <div className="container max-w-5xl text-center relative z-10">
          <motion.div 
            ...
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-700 text-xs font-semibold uppercase tracking-wider mb-8 shadow-sm"
          >
          ...
          >
            <Link to="/login?tab=signup" className="w-full sm:w-auto overflow-hidden rounded-md">
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button size="lg" className="w-full sm:w-auto rounded-md h-14 px-8 text-base gap-2 bg-indigo-600 hover:bg-indigo-700 text-white shadow-md hover:shadow-lg transition-all">
                  Start Practicing <ArrowRight className="h-5 w-5" />
                </Button>
              </motion.div>
            </Link>
            <Link to="/login" className="w-full sm:w-auto rounded-md">
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
      ...
      <section className="px-4 pb-24 relative z-10">
        <div className="container max-w-6xl">
          <FadeUp delay={0.4}>
            <div className="rounded-2xl border border-slate-200/60 bg-white/50 backdrop-blur-sm p-4 shadow-2xl relative transition-transform hover:rotate-1 duration-700">
      ...
      {/* Features */}
      <section className="py-24 px-4 relative z-10 bg-white border-y border-slate-200 overflow-hidden">
